...
目的: 最後のリリースタグ以降のユーザー向け変更をキュレートし、このリポジトリの `CHANGELOG.md`（Unreleased セクション）に記録する。`/update-changelog` プロンプト、リリースノートの運用ガイド、共有 AGENTS ノートを踏まえた手順。
...
- 対象ファイル: リポジトリの `CHANGELOG.md`（アプリ固有 changelog がある場合は、そのアプリに関係する項目だけを入れる）
...
3) **エントリをキュレート（ユーザー向けのみ）**
   ...
   - アプリ固有 changelog では、必ずそのアプリに関係するユーザー向け変更だけを記録する
...
5) **サニティチェック**
   ...
   - リリース直後は、リポジトリ固有のリリースガイドに従って、次のパッチ用の新しい `Unreleased` セクションを開始する
