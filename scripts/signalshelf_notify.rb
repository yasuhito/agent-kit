...
def normalize_content(value)
  ...
  if value.is_a?(Array)
    value.map { |item| item.is_a?(Hash) ? item["text"] : item.to_s }.compact.join("\n")
  else
    value.to_s
  end
end
...
def extract_agent_type(text)
  ...
  match = text.match(/\[AGENT:([^\]]+)\]/)
  return match[1] if match

  match = text.match(/🗣️\s*\*{0,2}([A-Za-z0-9_-]+):?\*{0,2}\s*/i)
  return match[1] if match

  match = text.match(/Sub-agent\s+([A-Za-z0-9_-]+)\s+completed/i)
  return match[1] if match

  match = text.match(/(?:^|\n)\s*([A-Za-z0-9_-]+)\s+completed\b/i)
  return match[1] if match

  match = text.match(/Agent:\s*([A-Za-z0-9_-]+)/)
  return match[1] if match
...
def write_capture(content, root:, category:, timestamp:, agent_label:, description:)
...
def run_signalshelf_notify
  ...
  agent_type ||= task_result[:agent_type] if task_result
  ...
  completion = task_result[:description].to_s if completion.to_s.empty? && task_result && task_result[:description]
  ...
  frontmatter = build_frontmatter(
    ...
    "task_subagent_type" => task_result ? task_result[:agent_type] : nil,
  ...
  if task_result
    metadata_lines << "**Task Description:** #{task_result[:description]}" if task_result[:description]
    metadata_lines << "**Task Subagent:** #{task_result[:agent_type]}" if task_result[:agent_type]
    metadata_lines << "**Task Background:** #{task_result[:run_in_background]}" unless task_result[:run_in_background].nil?
    metadata_lines << "**Task Call ID:** #{task_result[:call_id]}" if task_result[:call_id]
  end
